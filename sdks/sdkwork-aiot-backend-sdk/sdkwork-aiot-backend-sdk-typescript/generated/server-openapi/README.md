...
```bash
npm install @sdkwork/aiot-backend-sdk
# or
yarn add @sdkwork/aiot-backend-sdk
# or
pnpm add @sdkwork/aiot-backend-sdk
```

## Quick Start

```typescript
import { SdkworkBackendClient } from '@sdkwork/aiot-backend-sdk';

const client = new SdkworkBackendClient({
  baseUrl: '/backend/v3/api/iot',
  timeout: 30000,
});
...
```typescript
const client = new SdkworkBackendClient({ baseUrl: '/backend/v3/api/iot' });
client.setApiKey('your-api-key');
